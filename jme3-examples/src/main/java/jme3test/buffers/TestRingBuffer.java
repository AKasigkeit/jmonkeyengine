 ...
 */
public class TestRingBuffer extends TestUtil.AutoScreenshotApp {
    ...
    public void simpleInitApp() {
        ...
        super.simpleInitApp();
        Random r = new Random(83764615L);
        for (int i = 0; i < SEEDS.length; i++) {
            SEEDS[i] = (r.nextFloat() * SCREEN_RANGE);
        }
    ...
    private class TimingProcessor extends TestUtil.NullProcessor {
        ...
        public void preFrame(float tpf) {
            ...
            if (profiler != null) profiler.spStep(SpStep.ProcPreFrame, getClass().getSimpleName(), "update and stream "+((SEEDS.length * 60) / (1024 * 1024))+" MB");
            for (int i = 0; i < SEEDS.length; i++) {
